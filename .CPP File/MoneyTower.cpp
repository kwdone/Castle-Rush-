...
MoneyTower::MoneyTower(SDL_Renderer* renderer, Vector2D setPos) :
	pos(setPos), angle(0.0f), timerWeapon(4.0f) {
	textureMain = TextureLoader::loadTexture(renderer, "1P_MoneyTower.bmp");
}



void MoneyTower::update(SDL_Renderer* renderer, float dT) {
	//Update timer.
	timerWeapon.countDown(dT);

	increaseMoney(renderer);
}


void MoneyTower::increaseMoney(SDL_Renderer* renderer) {
...
void MoneyTower::draw(SDL_Renderer* renderer, int tileSize) {
...
void MoneyTower::drawWithoutAnimation(SDL_Renderer* renderer, SDL_Texture* textureSelected,
	int offset, int tileSize) {
	if (renderer != nullptr && textureSelected != nullptr) {
		//Draw the image at the turret's position and angle and offset.
		int w, h;
...
bool MoneyTower::checkIfOnTile(int x, int y) {
